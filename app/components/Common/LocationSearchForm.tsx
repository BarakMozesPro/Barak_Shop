...
const LocationSearchForm = () => {
  ...
  useEffect(() => {
    if (location) {
      // search for place name using mapbox API for Israel
      const endpoint = `https://api.mapbox.com/geocoding/v5/mapbox.places/${location.longitude},${location.latitude}.json?proximity=31.7886443431394,34.78956294963248&country=IL&access_token=${process.env.NEXT_PUBLIC_MAPBOX_TOKEN}`;
      fetch(endpoint)
  ...
  const handleChange = async (event: ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      const endpoint = `https://api.mapbox.com/geocoding/v5/mapbox.places/${event.target.value}.json?proximity=31.7886443431394,34.78956294963248&country=IL&access_token=${process.env.NEXT_PUBLIC_MAPBOX_TOKEN}&autocomplete=true`;
